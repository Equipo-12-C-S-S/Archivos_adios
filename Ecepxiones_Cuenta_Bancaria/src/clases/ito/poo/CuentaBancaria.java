...
public class CuentaBancaria implements Serializable, Comparable<CuentaBancaria> {
	...
	private void verificaCuenta(long numCuenta)throws NumCuentaException {
		if( numCuenta<9999)
			throw new NumCuentaException("el número de cuenta debe ser un mayor a 9999");	
	}
	private void verificaSaldo(float saldo)throws  SaldoException{
		if(saldo<5000)
        	throw new SaldoException("El saldo minimo para la apertura de la cuenta es de $5,000.00");
	}
	public CuentaBancaria(long numCuenta, String nomCliente, float saldo, LocalDate fechaApertura,
			LocalDate fechaActualizacicon) throws NumCuentaException ,SaldoException{
		super();
	...
	public boolean retiro(float cantidad) throws ExcepcionRetiro,RetiroException{
		...
		if (cantidad>=100 && cantidad<=6000 && cantidad%100==0) {
			if (this.saldo >= cantidad) {
		    	...
		    	throw new RetiroException("Los fondos son insuficientes");}
		}else 
				throw new ExcepcionRetiro("Error: la cantidad debe ser de $100 en $100 y maximo hasta $6000 de retiro");
		
		return retiro;
	}
	public boolean Deposito(float cantidad) throws DepositoException {
		...
		if (cantidad>=500 && cantidad<=25000) {
			...
			throw new DepositoException("La cantidad a depositar debe ser entre $500 y $25000");
		return deposito;
	}
	...
	public void setNumCuenta(long numCuenta) throws NumCuentaException {
		verificaCuenta(numCuenta);
	    this.numCuenta = numCuenta;
	}
